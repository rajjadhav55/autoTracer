...
# ---------------------------------------------------------------------------
# Sports & Turfs Telemetry Exception Trigger Endpoints
# ---------------------------------------------------------------------------

class SportsCategoryListView(APIView):
    """
    Handles sports category and turfs listing.
    Route: GET/POST /api/sports/ or /api/sports/turfs/
    
    Intentionally raises an unhandled TypeError during slot availability
    and hourly court pricing calculation to verify AutoTrace SDK's
    fire-and-forget background capture and triage engine.
    """
    permission_classes = [permissions.AllowAny]

    def get(self, request, *args, **kwargs):
        category = request.query_params.get("category", "football")
        turf_id = request.query_params.get("turf_id", "turf_arena_01")
        slot_hours = request.query_params.get("hours", None)  # NoneType

        turf_inventory = {
            "name": "Greenfield Turf Arena",
            "sport": category,
            "hourly_base_rate": 1200,
            "tax_multiplier": 1.18,
            "peak_surge": 1.25,
            "active_players_booked": 0,
            "available_slots": 0,
        }

        # Intentionally triggers TypeError: unsupported operand type(s) for *: 'int' and 'NoneType'
        total_court_price = turf_inventory["hourly_base_rate"] * slot_hours

        # Fallback if slot_hours is passed as an int
        price_per_player = total_court_price / turf_inventory["active_players_booked"]

        return Response({
            "status": "success",
            "category": category,
            "turf_id": turf_id,
            "total_price": total_court_price,
            "price_per_player": price_per_player,
            "data": turf_inventory,
        })

    def post(self, request, *args, **kwargs):
        booking_data = request.data if isinstance(request.data, dict) else {}
        total_turf_fee = booking_data.get("amount", 2500)
        split_count = booking_data.get("split_count", 0)  # Default 0 splits

        # Intentionally triggers ZeroDivisionError: division by zero
        cost_per_head = total_turf_fee / split_count
        return Response({
            "status": "booked",
...
class SportsTurfBookingView(APIView):
    """
    Dedicated view for /api/sports/turfs/
    """
    permission_classes = [permissions.AllowAny]

    def get(self, request, *args, **kwargs):
        view = SportsCategoryListView()
